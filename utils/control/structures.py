...
def convert_str_of_list_to_list(df: pd.DataFrame, col_to_convert: str) -> pd.DataFrame:
    ...
    if col_to_convert not in df.columns:
        raise KeyError(f"Column '{col_to_convert}' not found in DataFrame. "
...
def are_lists_equal(list_a: List[str], list_b: List[str]) -> bool:
    ...
    if len(only_in_a) > 0:
        print(
            f"Elements present in first list but not in second: \n{only_in_a}"
        )
    if len(only_in_b) > 0:
        print(
            f"Elements present in second list but not in first: \n{only_in_b}"
        )
